    ...
    <div class="py-6">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <div class="rounded-3xl border border-black/5 bg-white p-8 shadow-sm">
                ...
                <form method="POST" action="{{ route('barbers.update', $barber) }}" enctype="multipart/form-data"
                    class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input name="name" value="{{ old('name', $barber->name) }}" required
                    ...
                    @if ($barber->photo)
                        <div>
                            <label class="block text-sm font-semibold text-gray-700">Foto actual</label>
                            <img src="{{ asset('storage/' . $barber->photo) }}" alt="{{ $barber->name }}"
                                class="mt-2 h-24 w-24 rounded-2xl object-cover border border-gray-200">
                        </div>
                    @endif

                    <div>
                        <label class="block text-sm font-semibold text-gray-700">Nueva foto (opcional)</label>
                    ...
                    <div>
                        ...
                        <textarea name="short_description" rows="3"
                            class="mt-2 block w-full rounded-xl border-gray-300 shadow-sm focus:border-[#c9a227] focus:ring-[#c9a227]">{{ old('short_description', $barber->short_description) }}</textarea>
                    ...
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $barber->is_active))
